...
class Calculator {
    ...
    public String calculate(String expression) {
        ...
        String normalExpression = normalizer.normalizeExpression(expression);

        if (!validator.validate(normalExpression)) {
            System.out.println("Выражение : недопустимая строка");
        } else {

            fillInputArray(normalExpression);

            fillRPNStack(inputArray);

            return countFromRPN().toString();
        }
        return "";
    }
